 ...
 */
public class TicketRepositoryBase extends RepositoryBase<Ticket> implements TicketRepository {
    @Override
    ...
    private Collection<Ticket> getTickets(List<Ticket> tickets, ResultSet result) throws SQLException {
        while (result.next()) {
            ...
            tickets.add(new Ticket.TicketBuilder(userId, message).setResolverId(resolverId).setStatus(status).setId(id).build());
        }
        return tickets;
    }
}
